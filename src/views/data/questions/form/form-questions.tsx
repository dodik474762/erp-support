...
const FormQuestionsViews = ({ base_url = "" }) => {
  ...
  const validation = (data: any): boolean => {
    ...
    if (data.type.value.toLowerCase() != "image") {
      if (data.questions == "") {
    ...
    if (data.answers.length == 0) {
      ...
      const rightAnswer = [];
      data.answers.forEach((elm: any) => {
        if (elm.is_right == true) {
          rightAnswer.push(elm);
        }
      });

      if (rightAnswer.length == 0) {
        setErrors({ message: "Jawaban yang benar Harus Diisi" });
        return false;
      }
    }
    ...
    if (validation(postData)) {
      ...
      answers.forEach((element: any) => {
        if (element.type.toLowerCase() == "image") {
          formData.append("files-answer-" + element.id, element.file);
        }
      });
  ...
  return (
    <>
      <PageTitle titlePage="Data" subTitle="Questions Add" />
...
export default FormQuestionsViews;
